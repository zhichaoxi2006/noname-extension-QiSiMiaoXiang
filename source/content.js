...
export async function content(config, pack) {
            ...
            ResistanceSkillDelete: function (key) {
                ...
                try {
                    ...
                    if (lib.translate[key + '_info']) {
                        lib.translate[key + '_info'] = '<ins>此技能已被无效化</ins>' + '<del>' + lib.translate[key + '_info'] + '</del>';
            ...
            defineProperty_qsmx: function () {
                ...
                for (let index = 0; index < skills.length; index++) {
                    ...
                    if (key.startsWith('qsmx_') || key.startsWith('_qsmx_')) {
                        Object.defineProperty(lib.skill, key, { writable: false, configurable: false });
                    }
                }
                for (let index = 0; index < character.length; index++) {
                    const key = character[index];
                    if (key.startsWith('qsmx_')) {
                        Object.defineProperty(lib.character, key, { writable: false, configurable: false });
                    }
                }
                for (let index = 0; index < translate.length; index++) {
                    const key = translate[index];
                    if (key.startsWith('qsmx_')) {
                        Object.defineProperty(lib.translate, key, { writable: false, configurable: false });
                    }
                }
            }
        }
    ...
    Object.assign(lib.element.content, {
        AntiResistanceDie: function () {
            ...
            "step 1";
            if (player.AntiResistanceDieAfter) player.AntiResistanceDieAfter(source);
            "step 2";
            ...
            "step 4";
            if (player.AntiResistanceDieAfter2) player.AntiResistanceDieAfter2(source);
            "step 5";
            game.broadcastAll(function (player) {
                if (game.online && player == game.me && !_status.over && !game.controlOver && !ui.exit) {
                    if (lib.mode[lib.configOL.mode].config.dierestart) {
    ...
    Object.assign(lib.skill, {
        _qsmx_DieResistance: {
            ...
            filter: function (event, player) {
                if (lib.characterSort.mode_extension_奇思妙想.qsmx_HellOfResistance.includes(player.name)) return true;
                return false;
            },
            content: function () {
                trigger.cancel();
        ...
        _qsmx_gameStart: {
            ...
            firstDo: true,
            filter: function (event, player) {
                return lib.config.skill_delete || game.hasPlayer(c => c.name == 'qsmx_mimidog' || c.name1 == 'qsmx_mimidog' || c.name2 == 'qsmx_mimidog');
            },
            content: function () {
                var skills = Object.keys(lib.skill);
                var character = Object.keys(lib.character);
                ...
                for (let index = 0; index < players.length; index++) {
                    ...
                    for (let index = 0; index < lib.qsmx.ResistanceSkills.length; index++) {
